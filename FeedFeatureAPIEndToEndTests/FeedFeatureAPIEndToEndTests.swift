//
//  FeedFeatureAPIEndToEndTests.swift
//  FeedFeatureAPIEndToEndTests
//
//  Created by Srinivasan Rajendran on 2020-12-21.
//

import XCTest
import FeedFeature

class FeedFeatureAPIEndToEndTests: XCTestCase {

    func test_endToEndAPITestServerGETFeedResult_matchesFixedTestAccountData() {
        let receivedResult = getFeedResult()
        switch receivedResult {
        case let .success(feedItems)?:
            XCTAssertEqual(feedItems.count, 8, "Expected items in the test account feed")
    ...
    private func getFeedResult(file: StaticString = #filePath, line: UInt = #line) -> FeedResult? {
        ...
        let exp = expectation(description: "wait for feed completion")
        remoteFeedLoader.loadFeed { result in
